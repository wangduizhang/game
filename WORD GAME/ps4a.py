...
def isValidWord(word, hand, wordList):
    ...
    for i in worddic:
        if i in hand:
            ...
            s = False
            break
    if ( word.lower() in wordList ) and s:
        return True
...
def playHand(hand, wordList, n):
    ...
    while True:
        ...
        while True:
            if isValidWord(order, newhand, wordList):
                print '正确的单词！'
                word = order
                sumpoint += getWordScore(word, n)
                break
            else:
